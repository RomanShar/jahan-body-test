...
import { Heart, Brain, Wind, Zap, Droplets, Footprints, CheckCircle, ChevronRight } from 'lucide-react';
...
type Stage = 'intro' | 'breathing' | 'scanning' | 'results';
      ...
      <div className="max-w-4xl mx-auto p-4 sm:p-8">
        ...
        <div className="bg-gradient-to-br from-blue-50 to-purple-50 p-6 rounded-xl mb-6 border-2 border-purple-200">
          ...
          <p className="text-gray-700 leading-relaxed">
            {parseFloat(avgScore) >= 8 && "Отличный результат! Вы обладаете высоким уровнем телесной осознанности. Продолжайте практику для поддержания этого состояния."}
            {parseFloat(avgScore) >= 6 && parseFloat(avgScore) < 8 && "Хороший результат! У вас есть базовая связь с телом, но есть зоны, требующие внимания. Регулярная практика поможет углубить осознанность."}
            {parseFloat(avgScore) >= 4 && parseFloat(avgScore) < 6 && "Средний результат. Многие зоны находятся в состоянии умеренного блока. Это нормально для начинающих. Фокусируйтесь на практиках для проблемных зон."}
            {parseFloat(avgScore) < 4 && "Вы находитесь в начале пути осознанности тела. Не переживайте - осознание блоков уже первый шаг к их раскрытию. Начните с ежедневной 10-минутной практики."}
          </p>
        </div>
        ...
        <div className="bg-gradient-to-r from-purple-600 to-pink-600 p-6 rounded-xl mb-6 text-white shadow-lg">
          ...
          <p className="mb-4 opacity-90">
            ...
            Вместе мы разберём ваши результаты и составим персональный план работы с блоками.
          </p>
          <button
